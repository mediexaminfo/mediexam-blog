<script lang="ts" context="module">
	...
	export async function load({ page, fetch, session, context }) {
		...
		const query = `*[_type == "post" && category->slug.current == $slug]{title, description, publishedAt, "author": author->name, "authorSlug":author->slug.current, "slug": slug.current, "category": category->title, "categorySlug": category->slug.current}`;
